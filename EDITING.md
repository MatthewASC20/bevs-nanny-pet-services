...
`bev-portrait.jpg`, which lives in this folder next to `index.html`.
...
2. In `content.json`, find the `"portrait"` line and change `"photo"` to the new
   ...
   ```json
   "portrait": { "initials": "Bev", "note": "add your photo here", "photo": "bev-portrait.jpg", "photoAlt": "Beverly de Jesus — Granny Bev" }
   ```
...
### The `"items": []` list is empty right now

The Pets section is **hidden** until you add a real pet, so the website never shows
half-finished cards. The three example blocks are parked just below, in
`"exampleItems"` — the website ignores everything in there.

When you're ready, copy one block out of `"exampleItems"` into `"items"`, change the
name and the note, and the section appears on the site automatically:

```json
"items": [
  { "name": "Rex", "note": "Golden retriever — our daily park buddy.", "folder": "pet-1", "photoAlt": "Rex, a golden retriever", "photos": ["1.jpg"] }
],
```

The same applies to the Children section: empty the `"items"` list and it hides too.
